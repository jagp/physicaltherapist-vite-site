    ...
    <div>
      ...
      <section className={s.section}>
        <div className={s.grid}>
          <Card tone="surface" padding="36px">
            {sent ? (
              <div className={s.sent}>
                <img src={leafLeaves} alt="" className={s.sentLeaf} width={400} height={976} />
                <h3 className={s.sentTitle}>Thank you</h3>
                <p className={s.sentBody}>We'll be in touch within one business day to schedule your visit.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className={s.form}>
                <div className={s.pair}>
                  <Input label="Full name" placeholder="Jane Doe" required value={form.name} onChange={update('name')} />
                  <Input label="Phone" placeholder="(508) 000-0000" value={form.phone} onChange={update('phone')} />
                </div>
                <Input label="Email" type="email" placeholder="you@email.com" required value={form.email} onChange={update('email')} />
                <Input
          ...
          <div>
            <Card tone="plum" padding="32px" leaf leafSrc={leafLeaves} className={s.visitCard}>
            ...
            <Card tone="cream" padding="24px">
              ...
              <p className={s.hoursBody}>New patients welcome. We'll find a time that works for you.</p>
            </Card>
          </div>
        </div>
      </section>
