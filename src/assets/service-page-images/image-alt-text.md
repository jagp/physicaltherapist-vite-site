# Service image alt text (staging)

Canonical `{slug}-header.png` / `{slug}-inset.png` files and their draft alt text.
Alt text drops into `ServiceImage.alt` in `src/data/services.ts` once each service has a
`content` block. Descriptive (screen-reader first), not keyword-stuffed. Edit freely.

| File | Role | Draft alt text |
|---|---|---|
| `pelvic-floor-bladder-health-header.png` | header | A physical therapist speaks warmly with a couple during a consultation in a calm, home-like room. |
...
| `menopause-midlife-health-header.png` | header | A woman practices a standing strength and posture exercise with her physical therapist's guidance. |
| `menopause-midlife-health-inset.png` | inset | A woman does a supported step-up balance exercise to stay strong and steady through midlife. |
| `pregnancy-landscape.png` | header | _(already wired in services.ts)_ |
| `pregnancy-inset.png` | inset | _(already wired in services.ts)_ |
